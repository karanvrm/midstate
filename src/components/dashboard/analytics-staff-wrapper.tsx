...
import { AnalyticsStaffClient } from "@/components/dashboard/analytics-staff-client";
import {
  Select,
  SelectContent,
  SelectItem,
...
interface AnalyticsStaffWrapperProps {
  initialData: StaffAnalyticsData;
}
...
export function AnalyticsStaffWrapper({ initialData }: AnalyticsStaffWrapperProps) {
  ...
  useEffect(() => {
    ...
    async function loadAnalytics() {
      ...
      try {
        const response = await fetch(`/api/analytics/staff?filter=${filter}`, {
        ...
        const nextData = (await response.json()) as StaffAnalyticsData;
        setData(nextData);
      } catch (err) {
  ...
  return (
    <>
      ...
      <AnalyticsStaffClient data={data} headerAction={filterControl} />
    </>
  );
}
